...
public float step =2;
    ...
    void Update()
    {
		...
		if(transform.position.x == 6*step && transform.position.y == 9*step){
			transform.position = new Vector3(0,5*step,0);
			Debug.Log("LLLLLBBBB");
		}
		else if(transform.position.x == 0 && transform.position.y == 5*step){
			transform.position = new Vector3(6*step,0,0);
			Debug.Log("BBBBBRRRRRR");
		}
		else if(transform.position.x == 6*step && transform.position.y == 0){
			transform.position = new Vector3(6*step,9*step,0);
			Debug.Log("RRRRRWWWWWWWWWLLLLL");
		}
	}
    }
}
